...
#else
    //RTC_SetPrescaler(4000 - 1); /* RTC period = RTCCLK/RTC_PR = (32.768 KHz)/(32767+1) */
    RTC_SetPrescaler(0); // Счет каждой 25 мкс
#endif

    /* Wait until last write operation on RTC registers has finished */
    RTC_WaitForLastTask();

    // Длина суперфрейма 100 мс. 0.1 * 40000 = 4000
    RTC_SetAlarm(0xFFFFFFFF);
    RTC_WaitForLastTask();
}

void rtcSetCounter(uint32_t cnt)
{
	RTC_WaitForLastTask();
	RTC_SetCounter(cnt);
}

void rtcSetAlarm(uint32_t cnt)
{
	RTC_WaitForLastTask();
	RTC_SetAlarm(cnt);
}

void rtcSetInterruptCb(void (*secIntCb)(uint32_t cnt),
                       void (*alrmIntCb)(uint32_t cnt))
{
	rtcSecIntCb = secIntCb;
	rtcAlrmIntCb = alrmIntCb;
...
{
	GPIO_WriteBit(GPIOA, GPIO_Pin_2, enable);
}
