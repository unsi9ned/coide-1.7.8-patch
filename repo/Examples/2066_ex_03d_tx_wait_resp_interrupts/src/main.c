...
static void rx_ok_cb(const dwt_cb_data_t *cb_data)
{
    ...
    if (cb_data->datalength <= FRAME_LEN_MAX)
    {
        ...
        printf("function = 0x%02x, ", rx_buffer[cb_data->datalength - 6]);
        printf("activity = 0x%02x, ", rx_buffer[cb_data->datalength - 5]);
        printf("blink rate = 0x%04x ", *(uint16_t*)&rx_buffer[cb_data->datalength - 4]);
        printf("(0x%04x)", *(uint16_t*)&rx_buffer[cb_data->datalength - 2]);
        printf("\r\n");
    }
