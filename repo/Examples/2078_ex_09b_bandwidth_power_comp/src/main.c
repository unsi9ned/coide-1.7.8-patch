...
struct ref_values {
    uint8 PGdly;
    uint32 power;
    int raw_temperature; // the DW IC raw register value, which needs conversion if you want a В°C value
    uint16 count;
};
 ...
 *    of a tag blinking at a high rate: the blink frame length is around 180 Вµs in this configuration, emitted once per millisecond. In this configuration,
