    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            if (frame_len <= FRAME_LEN_MAX)
            {
                ...
                printf("Frame received #%i:\t", rx_buffer[1]);
                ...
                printf("(0x%04x)\r\n", *(uint16_t*)&rx_buffer[frame_len - 2]);
            }
            ...
            printf("Std noise: %i\r\n", rx_diag.stdNoise);
            printf("Max noise: %i\r\n", rx_diag.maxNoise);
            printf("First path amp1: %i\r\n", rx_diag.firstPathAmp1);
            printf("First path amp2: %i\r\n", rx_diag.firstPathAmp2);
            printf("First path amp3: %i\r\n", rx_diag.firstPathAmp3);
            printf("Max growth CIR: %i\r\n", rx_diag.maxGrowthCIR);
            printf("Rx preamble symb. count: %i\r\n", rx_diag.rxPreamCount);
            printf("First path: %i\r\n", rx_diag.firstPath);
            printf("\r\n");
