    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            if (frame_len <= FRAME_LEN_MAX)
            {
                ...
                printf("(0x%04x)\r\n", *(uint16_t*)&rx_buffer[frame_len - 4]);
            }
            ...
            if ((frame_len == 14) && (rx_buffer[0] == 0xC5) && (rx_buffer[10] == 0x43) && (rx_buffer[11] == 0x2))
            {
                ...
                printf("Frame sent #%i to '", tx_msg[DATA_FRAME_SN_IDX]);

                for (i = 0; i < 8; i++)
				{
                	printf("%c%c", tx_msg[DATA_FRAME_DEST_IDX + i], i == 7 ? '\'' : ' ');
				}
                printf(" from '%c%c' ", tx_msg[DATA_FRAME_DEST_IDX + 8], tx_msg[DATA_FRAME_DEST_IDX + 9]);
                printf("\r\n");
