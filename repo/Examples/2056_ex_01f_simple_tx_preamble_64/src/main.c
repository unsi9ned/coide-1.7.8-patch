/*! ----------------------------------------------------------------------------
 *  @file    main.c
 *  @brief   Simple TX example code with 64-symbol preamble length
 *
 * @attention
 *
...
#define APP_NAME "SIMPLE TX PLEN64 v1.2"
...
int main(void)
{
    ...
    while(1)
    {
        ...
        printf("Frame sent #%i:\t", tx_msg[BLINK_FRAME_SN_IDX]);

        for(int i = 2; i < (sizeof(tx_msg) - 2); i++)
        {
        	printf("'%c' ", tx_msg[i]);
        }
        printf("\r\n");
