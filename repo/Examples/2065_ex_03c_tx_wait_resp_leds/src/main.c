...
int main(void)
{
    ...
    while (1)
    {
        ...
        printf("\r\nFrame sent #%i:\t", tx_msg[BLINK_FRAME_SN_IDX]);

        for(int i = 2; i < (sizeof(tx_msg) - 4); i++)
        {
        	printf("'%c' ", tx_msg[i]);
        }
        printf("\r\n");
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            if (frame_len <= FRAME_LEN_MAX)
            {
                ...
                printf("Frame received #%i ", rx_buffer[DATA_FRAME_SN_IDX]);
                printf("from '%c%c' ", rx_buffer[DATA_FRAME_DEST_IDX + 8], rx_buffer[DATA_FRAME_DEST_IDX + 9]);
                printf("function = 0x%02x, ", rx_buffer[frame_len - 6]);
                printf("activity = 0x%02x, ", rx_buffer[frame_len - 5]);
                printf("blink rate = 0x%04x ", *(uint16_t*)&rx_buffer[frame_len - 4]);
                printf("(0x%04x)", *(uint16_t*)&rx_buffer[frame_len - 2]);
                printf("\r\n");
            }
