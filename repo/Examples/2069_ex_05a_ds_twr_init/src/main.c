...
int main(void)
{
    ...
    while (1)
    {
        ...
        printf("\r\nSend POLL message #%i:\t", tx_poll_msg[ALL_MSG_SN_IDX]);
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            uint8_t respMessageIdx = rx_buffer[ALL_MSG_SN_IDX];
            ...
            if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                printf("RESPONSE message received #%i: ", respMessageIdx);

                rx_buffer[ALL_MSG_SN_IDX] = respMessageIdx;
        		for(int i = 0; i < sizeof(rx_resp_msg); i++)
        		{
                ...
                if (ret == DWT_SUCCESS)
                {
                    printf("Send FINAL message #%i: ", tx_final_msg[ALL_MSG_SN_IDX]);

            		for(int i = 0; i < sizeof(tx_final_msg); i++)
            		{
            			printf("%02X ", tx_final_msg[i]);
            		}
            		printf("\r\n");
                ...
                {
                	printf("Send FINAL message error\r\n");
                }
            }
        }
        else
