...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            uint8_t pollMessageIdx = rx_buffer[ALL_MSG_SN_IDX];
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                printf("\r\nPOLL message received #%i: ", pollMessageIdx);

                rx_buffer[ALL_MSG_SN_IDX] = pollMessageIdx;
                ...
                if (ret == DWT_ERROR)
                {
                	printf("Send RESPONSE message error\r\n");
                    continue;
                }

                printf("Send RESPONSE message #%i:\t", tx_resp_msg[ALL_MSG_SN_IDX]);

        		for(int i = 0; i < sizeof(tx_resp_msg); i++)
        		{
        			printf("%02X ", tx_resp_msg[i]);
        		}
        		printf("\r\n");
                ...
                if (status_reg & SYS_STATUS_RXFCG)
                {
                    ...
                    if (frame_len <= RX_BUF_LEN)
                    {
                        ...
                        printf("FINAL message received #%i: ", rx_buffer[ALL_MSG_SN_IDX]);

						for(int i = 0; i < sizeof(rx_final_msg); i++)
						{
							printf("%02X ", rx_buffer[i]);
						}
						printf("\r\n");
                ...
                else
                {
                    ...
                    if(status_reg & SYS_STATUS_ALL_RX_TO)
                    	printf("Wait FINAL message timeout\r\n");
